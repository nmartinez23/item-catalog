...
from database_setup import Base, Category, LatestItem, User
...
def categoryItemJSON(category_id, item_id):
    Latest_Item = session.query(LatestItem).filter_by(id=item_id).one()
...
def showCategories():
    categories = session.query(Category).all()
    items = session.query(LatestItem).all()
...
def newItem(category_id):
    ...
    if request.method == 'POST':
        item = LatestItem(name=request.form['name'], description=request.form[
...
def editItem(category_id, item_id):
    if 'username' not in login_session:
        return redirect('/login')
    editedItem = session.query(LatestItem).filter_by(id=item_id).one()
    category = session.query(Category).filter_by(id=category_id).one()
    if login_session['user_id'] != category.user_id:
...
def deleteItem(category_id, item_id):
    if 'username' not in login_session:
        return redirect('/login')
    category = session.query(Category).filter_by(id=category_id).one()
    itemToDelete = session.query(LatestItem).filter_by(id=item_id).one()
    if login_session['user_id'] != category.user_id:
